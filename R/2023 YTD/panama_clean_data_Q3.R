...
country_name <- "Panama"
raw_data_ridl <- 'https://ridl.unhcr.org/dataset/db744a82-4916-4fa3-874f-731f1c60b3df/resource/f750879c-f189-4ee2-a1ee-fcda6dded380/download/survey-xlsx-data__ak73pat2swlum4unfvy3yr_data.xlsx'   
...
df_wrangle <- df_wrangle |> 
  filter(A003_date >= ymd("2023-07-01") & A003_date <= ymd("2023-09-30"))
    

# fix data collection country ---------------------------------------------

df_wrangle <- df_wrangle |> 
  mutate(a_introduction_a04_country = "Panama") 



# fix data collection location --------------------------------------------

df_wrangle <- df_wrangle |>
  mutate(
    A004_location = case_when(
      id == "30546683" & uuid == "acd87849-d722-4e48-8e63-74fa3b959a82" ~ "Canaan Membrillo",
      id == "30546612" & uuid == "151077d1-0fb5-4cde-bfca-e3d330732f9e" ~ "Canaan Membrillo",
      id == "30446149" & uuid == "a027afea-cd90-442c-97ef-35055934ebe0" ~ "Canaan Membrillo",
      id == "30446139" & uuid == "914f7403-0841-47d2-9d94-6941309c5aa0" ~ "Canaan Membrillo",
      id == "30446140" & uuid == "faf6d2cb-432c-4d80-9ed5-9bdd43abf276" ~ "Canaan Membrillo",
      id == "30446147" & uuid == "ee25d569-3022-4ad3-aef5-6beec6126d05" ~ "Canaan Membrillo",
      id == "30446144" & uuid == "ce48732a-a363-42f8-9b71-c4e3f9997aa7" ~ "Canaan Membrillo",
      id == "30446143" & uuid == "d4ca8a8c-3009-4da7-9b83-3c1fa722a692" ~ "Canaan Membrillo",
      id == "30446142" & uuid == "256a5d61-fc50-4647-ab1f-37bdd30fe3ee" ~ "Canaan Membrillo",
      TRUE ~ A004_location
    )
  ) 
...
file_name <- paste0(tolower(country_name), '_mm_questionnaire_q3_2023_clean.csv')
...
m <- resource_metadata(
  type = "data",
  url = file_name,
  name = paste0(country_name, ": Mixed movement Q3 2023 - Cleaned"),
