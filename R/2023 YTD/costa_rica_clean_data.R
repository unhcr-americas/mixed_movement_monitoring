...
raw_data_ridl <- 'https://ridl.unhcr.org/dataset/0216531e-1cbe-4dd6-81e3-e16237afad3b/resource/639362ab-b592-410a-9225-8df7067f75d5/download/survey-csv-data__adakemy9uvire3n5bnbajm_data.csv'   
...
df_wrangle <- df_raw |> 
  ...
  select(start,                                                                                                           
         ...
         B004_nationality_stateless = questionnaire_b_biodata_b004_nationality_stateless_2,
         ...
         C001_when_left_coo = questionnaire_b_biodata_c001_when_left_coo,
...
df_wrangle <- df_wrangle |> 
  mutate(B004_nationality = case_when(B004_nationality == "other" ~ B004_nationality_other,
                                      TRUE ~ B004_nationality))

# fix nationality ---------------------------------------------------------

df_wrangle <- df_wrangle |> 
  mutate(B004_nationality = case_when(B004_nationality == "PerÃº" ~ "Peru",
                                      B004_nationality == "AfganistÃ¡n" ~ "Afganistan",
                                      B004_nationality == "RepÃºblica Dominicana" ~ "Republica Dominicana",
                                      B004_nationality == "CamerÃºn" ~ "Cameroon",
                                      B004_nationality == "HaitÃ­" ~ "Haiti",
                                      B004_nationality == "Haiti­" ~ "Haiti",
                                      TRUE ~ B004_nationality),
         B004_nationality_other = case_when(B004_nationality != "other" ~ NA_character_,
                                            TRUE ~ B004_nationality_other))

# create dual nationality ---------------------------------------------

df_wrangle <- df_wrangle |> 
  mutate(B004_nationality = case_when(B004_nationality == "Ecuador Haiti" ~ "Ecuador;Haiti",
                                      B004_nationality == "Ecuador Venezuela" ~ "Ecuador;Venezuela",
                                      B004_nationality == "Nicaragua Venezuela" ~ "Nicaragua;Venezuela",
                                      TRUE ~ B004_nationality)) |> 
...
df_wrangle <- df_wrangle |>
  mutate(
    C001_when_left_cor = case_when(
      B004_nationality != B011_habitual_residence ~ C001_when_left_coo,
      TRUE ~ NA_character_
    ),
    C001_when_left_coo = case_when(
      B004_nationality == B011_habitual_residence ~ C001_when_left_coo,
      TRUE ~ NA_character_
    )#,
